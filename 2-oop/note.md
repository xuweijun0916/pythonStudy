...
- 私有
    ...
    - 在成员前面添加两个下划线即可
        ...
        class Person():
            ...
            # __age就是私有成员
            __age = 18
    - Python的私有不是真私有，是一种成为name mangling的改民策略，
...
- super
    ...
    - super与父类直接没任何实质关系，但通过super可以调用到父类
...
- 构造函数
    - 在对象进行实例化的时候，系统自动调用一个函数叫构造函数，通常此函数用来对实例化对象
...
- 多态不是语法，是一种设计思想
- 多态性：一种调用方式，不同的执行效果
- 多态：同一事物的多种形态，动物分为人、狗、猪

- Mixin设计模式
    - 主要采用多继承方式对类的功能进行扩展
    
- 我们使用多继承语法来实现Mixin
- 使用Mixin实现多继承表示某一单一功能，而不是某个物品
...
# 7.类的常用魔术方法
- 魔术方法就是不需要人为调用方法，基本是在特定的时刻自动触发
- 魔术方法的一个统一特征，方法名被前后各两个下划线包裹
- 操作符
    - __init__: 构造函数
    - __new__ : 对象实例化方法，此函数较特殊，一般不需要使用
    - __call__ : 对象当函数使用的时候来触发
    - __str__: 当对象被当作字符串使用的时候调用
    - __repr__: 返回字符串，跟‘__str__’具体区别自行学习
- 描述符相关
    - __set__
    - __get__
    - __delete__
